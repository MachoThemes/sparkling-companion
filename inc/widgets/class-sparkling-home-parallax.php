 ...
 * Sparkling Theme
 */
class Sparkling_Home_Parallax extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname' => 'sparkling_home_parallax',
			'description' => esc_html__( 'Sparkling FrontPage Parallax Section', 'sparkling' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'sparkling_home_parallax', esc_html__( '[Sparkling] Parralax Section For FrontPage', 'sparkling' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$image_pos = isset( $instance['image_pos'] ) ? $instance['image_pos'] : esc_html__( 'left', 'sparkling' );
						...
						<div class="<?php echo esc_attr( $class4 ); ?>">
							...
							if ( ( 'left' == $image_pos || 'top' == $image_pos ) && '' != $image_src ) { ?>
								<div class="<?php echo esc_attr( $class6 ); ?>">
									<img class="cast-shadow img-responsive" alt="<?php echo esc_attr( $title ); ?>" src="<?php echo esc_url( $image_src ); ?>">
								</div>
							<?php } ?>
	...
	function form( $instance ) {
		...
		<p class="sparkling-media-control" data-delegate-container="<?php echo esc_attr( $this->get_field_id( 'image_src' ) ) ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'image_src' ) ); ?>"><?php _e( 'Image', 'sparkling' ); ?>:</label>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'body_content' ) ); ?>"><?php esc_html_e( 'Content ', 'sparkling' ) ?></label>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'image_pos' ) ); ?>"><?php esc_html_e( 'Image Position ', 'sparkling' ) ?></label>
			<select name="<?php echo esc_attr( $this->get_field_name( 'image_pos' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'image_pos' ) ); ?>" class="widefat">
				<option value="left" <?php selected( $instance['image_pos'], 'left' ); ?>><?php _e( 'Left', 'sparkling' ); ?></option>
				<option value="right" <?php selected( $instance['image_pos'], 'right' ); ?>><?php _e( 'Right', 'sparkling' ); ?></option>
				<option value="top" <?php selected( $instance['image_pos'], 'top' ); ?>><?php _e( 'Top', 'sparkling' ); ?></option>
				<option value="bottom" <?php selected( $instance['image_pos'], 'bottom' ); ?>><?php _e( 'Bottom', 'sparkling' ); ?></option>
				<option value="background-full" <?php selected( $instance['image_pos'], 'background-full' ); ?>><?php _e( 'Background Full', 'sparkling' ); ?></option>
				<option value="background-small" <?php selected( $instance['image_pos'], 'background-small' ); ?>><?php _e( 'Background Small', 'sparkling' ); ?></option>
			</select>
		</p>

		<p><label for="<?php echo esc_attr( $this->get_field_id( 'button1' ) ); ?>"><?php esc_html_e( 'Button 1 Text ', 'sparkling' ) ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'button1_link' ) ); ?>"><?php esc_html_e( 'Button 1 Link ', 'sparkling' ) ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'button2' ) ); ?>"><?php esc_html_e( 'Button 2 Text ', 'sparkling' ) ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'button2_link' ) ); ?>"><?php esc_html_e( 'Button 2 Link ', 'sparkling' ) ?></label>
		...
		<div class="checkbox_switch">
				<span class="customize-control-title onoffswitch_label">
					<?php _e( 'Border bottom', 'sparkling' ); ?>
				</span>
